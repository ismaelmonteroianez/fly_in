...
class Simulation():
    ...
    def assign_paths(self) -> list[list[Hub]]:
        ...
        if not self.paths:
            return []
        if len(self.drones) > 1000:
            return self.build_minimum_distribution()
        paths_plus_one = []
        paths_plus_two = []
        ...
        if plus_two_distribution is not None and turns_two is not None:
            distributions.append((plus_two_distribution, turns_two))
